<script lang="ts">
	...
	async function copyAsHTML() {
		...
		const fullHTML =
			...
			escapeHTML(subject || 'Tutoring Update') +
			'</title></head>' +
	...
	function markdownToHTML(md: string): string {
		let html = md
			...
			.replace(
				/^### (.+)$/gm,
				'<h3 style="margin:16px 0 8px;font-size:16px;color:#1f2937;">$1</h3>'
			)
			.replace(
				/^## (.+)$/gm,
				'<h2 style="margin:20px 0 10px;font-size:18px;color:#1f2937;">$1</h2>'
			)
		...
		html =
			'<p style="margin:0 0 12px;color:#374151;font-size:15px;line-height:1.6;">' + html + '</p>';
		html = html.replace(
			/((?:<li[^>]*>.*?<\/li>)+)/g,
			'<ul style="margin:0 0 12px;padding-left:20px;">$1</ul>'
		);
...
{:else}
	<div class="space-y-4">
		...
		{#if tableMissing}
			<Card>
				<div
					class="rounded-lg bg-[var(--color-warning-bg)] p-4 text-sm text-[var(--color-warning-text)]"
				>
					<p class="font-medium">Database table not found</p>
					<p class="mt-1">
						Run <code class="rounded bg-[var(--color-surface)] px-1 font-mono text-xs"
							>supabase/migrations/006_parent_emails.sql</code
						> in the Supabase SQL Editor to create the parent_emails table.
					</p>
				</div>
			</Card>
		{/if}
			...
			<Card>
				<div class="space-y-3">
					<div class="flex items-center justify-between">
						...
						<div class="flex items-center gap-2">
							...
							<button
								...
								class="rounded-md px-2.5 py-1 text-xs font-medium transition-colors {mode ===
								'preview'
									? 'bg-[var(--color-primary-100)] text-[var(--color-primary-700)]'
									: 'text-[var(--color-text-tertiary)] hover:bg-[var(--color-surface-secondary)]'}"
							>
					...
					<input
						...
						class="w-full rounded-lg border border-[var(--color-border)] bg-[var(--color-surface-primary)] px-3 py-2 text-sm text-[var(--color-text-primary)] placeholder-[var(--color-text-tertiary)] focus:border-[var(--color-primary-400)] focus:ring-1 focus:ring-[var(--color-primary-400)] focus:outline-none"
					/>

					{#if mode === 'edit'}
						<div
							class="flex flex-wrap gap-1 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface-secondary)] px-2 py-1.5"
						>
							<button
								onclick={() => insertMarkdown('**$1**', 'bold text')}
								class="rounded px-2 py-1 text-xs font-bold text-[var(--color-text-secondary)] transition-colors hover:bg-[var(--color-surface-elevated)] hover:text-[var(--color-text-primary)]"
								title="Bold (Ctrl+B)"
							>
								B
							</button>
							<button
								...
								class="rounded px-2 py-1 text-xs text-[var(--color-text-secondary)] italic transition-colors hover:bg-[var(--color-surface-elevated)] hover:text-[var(--color-text-primary)]"
								title="Italic (Ctrl+I)"
							>
								I
							</button>
							...
							<button
								...
								class="rounded px-2 py-1 text-xs font-semibold text-[var(--color-text-secondary)] transition-colors hover:bg-[var(--color-surface-elevated)] hover:text-[var(--color-text-primary)]"
								title="Heading 2"
							>
								H2
							</button>
							<button
								...
								class="rounded px-2 py-1 text-xs text-[var(--color-text-secondary)] transition-colors hover:bg-[var(--color-surface-elevated)] hover:text-[var(--color-text-primary)]"
								title="Heading 3"
							>
								H3
							</button>
							...
							<span class="mx-0.5 w-px bg-[var(--color-border)]"></span>
							<button
								onclick={() => insertMarkdown('[$1](https://...)', 'link text')}
								class="rounded px-2 py-1 text-xs text-[var(--color-text-secondary)] transition-colors hover:bg-[var(--color-surface-elevated)] hover:text-[var(--color-text-primary)]"
								title="Insert link"
							>
								&#128279;
						...
						<textarea
							...
							rows="16"
							class="email-editor-textarea w-full resize-y rounded-lg border border-[var(--color-border)] bg-[var(--color-surface-primary)] px-3 py-2.5 font-mono text-sm text-[var(--color-text-primary)] placeholder-[var(--color-text-tertiary)] focus:border-[var(--color-primary-400)] focus:ring-1 focus:ring-[var(--color-primary-400)] focus:outline-none"
						></textarea>
					{:else}
						<div
							class="min-h-[12rem] rounded-lg border border-[var(--color-border)] bg-[var(--color-surface-primary)] px-4 py-3 text-sm"
						>
							{#if body}
								{@html markdownToHTML(body)}
							{:else}
								<span class="text-[var(--color-text-tertiary)] italic">Nothing to preview yet.</span
								>
							{/if}
						</div>
					{/if}
