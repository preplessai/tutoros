...
{#each toast.toasts as t (t.id)}
	<div class={`flex items-center gap-3 px-4 py-3 rounded-xl border-2 shadow-clay-md animate-slide-in-right ${typeClasses(t.type)}`}>
		...
		<button onclick={() => toast.remove(t.id)} aria-label="Dismiss" class="shrink-0 p-0.5 rounded-lg hover:bg-black/5 transition-colors cursor-pointer">
