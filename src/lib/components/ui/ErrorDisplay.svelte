<script lang="ts">
	...
	let displayMessage = $derived(message || defaultMessages[status] || 'An unexpected error occurred.');
	...
	const defaultIcon = 'M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-2.694-.833-3.464 0L3.34 16.5c-.77.833.192 2.5 1.732 2.5z';
	let iconPath = $derived(iconPaths[status] || defaultIcon);
</script>

<div class={compact ? 'rounded-lg border border-[var(--color-border)] bg-[var(--color-surface-secondary)] p-6' : 'flex min-h-[60vh] items-center justify-center'}>
	<div class={compact ? '' : 'mx-auto max-w-md text-center'}>
		<!-- Icon -->
		<div class="mb-4 flex justify-center">
			<div class="flex h-16 w-16 items-center justify-center rounded-2xl {status === 403 || status === 401
		...
		<!-- Title -->
		<h2 class="{compact ? 'text-lg' : 'text-2xl'} font-[family-name:var(--font-heading)] text-[var(--color-text-primary)]">
			{displayTitle}
		</h2>
