<script lang="ts">
	...
	onMount(() => { dayPlanStore.fetchDay(dayId); });
</script>
	...
	<div class="space-y-6">
		<div>
			<div class="flex items-center gap-2">
				<h1 class="text-xl font-bold text-[var(--color-text-primary)]">{formatDateLong(dayPlanStore.currentDay.date)}</h1>
				{#if dayPlanStore.currentDay.energy_level}
					<Badge variant={dayPlanStore.currentDay.energy_level === 'high' ? 'success' : dayPlanStore.currentDay.energy_level === 'medium' ? 'info' : 'warning'}>
						{dayPlanStore.currentDay.energy_level}
					</Badge>
				{/if}
			</div>
			{#if dayPlanStore.currentDay.struggle_areas}
				<p class="text-sm text-[var(--color-text-secondary)] mt-1">Focus: {dayPlanStore.currentDay.struggle_areas}</p>
			{/if}
		</div>
		...
		<div class="flex gap-3 pt-4 border-t border-[var(--color-border)]">
			<Button variant="secondary" size="sm" onclick={() => showResourceSearch = !showResourceSearch}>
