	...
	<div class="flex items-start justify-between gap-3">
		<div class="min-w-0 flex-1">
			<div class="mb-1 flex items-center gap-2">
				<h3 class="truncate font-semibold text-[var(--color-text-primary)]">{plan.title}</h3>
				<Badge variant={plan.status === 'active' ? 'success' : 'default'}>{plan.status}</Badge>
			</div>
			<p class="text-sm text-[var(--color-text-secondary)]">
				{plan.grade} • {plan.subjects.slice(0, 3).join(', ')}
			</p>
			<div class="mt-3 flex items-center gap-3 text-xs text-[var(--color-text-tertiary)]">
		...
		<svg
			class="mt-2 h-5 w-5 shrink-0 text-[var(--color-text-tertiary)]"
			fill="none"
			viewBox="0 0 24 24"
			stroke="currentColor"
			><path
