<script lang="ts">
	...
	async function sendMessage() {
		...
		try {
			const hasCredits = creditStore.hasEnough(0.5);
			if (!hasCredits) {
				messages = [
					...messages,
					{
			...
			if (response.intent === 'info') {
				messages = [...messages, { role: 'assistant', content: response.message }];
			} else if (
				response.intent === 'edit_plan' ||
				response.intent === 'edit_day' ||
				response.intent === 'add_resources' ||
				response.intent === 'add_homework'
			) {
				messages = [...messages, { role: 'assistant', content: response.message }];
