	...
	try {
		...
		switch (event.type) {
			case 'checkout.session.completed': {
				...
				if (metadata.type === 'subscription') {
					...
					const creditsAmount = parseInt(
						env[`CREDITS_${(metadata.tier || 'pro').toUpperCase()}`] ||
							(metadata.tier === 'enterprise' ? '75' : metadata.tier === 'pro' ? '30' : '15')
					);
			...
			case 'invoice.paid': {
				...
				if (tier !== 'free') {
					const creditsAmount = parseInt(
						env[`CREDITS_${tier.toUpperCase()}`] ||
							(tier === 'enterprise' ? '75' : tier === 'pro' ? '30' : '15')
					);
					await supabaseRpc(supabaseUrl, serviceKey, 'refresh_credits', {
						p_user_id: userId,
						p_amount: creditsAmount
